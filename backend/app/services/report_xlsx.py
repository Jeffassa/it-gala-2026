...
def _kpi_style(cell, accent: bool = False) -> None:
    cell.font = Font(
        bold=True,
        size=22,
        color=ACCENT if accent else INK_LIGHT,
        name="Calibri",
    )
...
def build_report_xlsx(db: Session) -> bytes:
    ...
    ws["B3"] = (
        active_gala.name + " · " + str(active_gala.edition_year)
        if active_gala
        else "—"
    )
    ...
    total_revenue = float(
        db.scalar(select(func.coalesce(func.sum(Ticket.price), 0))) or 0
    )
    ...
    kpis = [
        (
            "Recettes totales",
            f"{int(total_revenue):,}".replace(",", " ") + " FCFA",
            True,
        ),
    ...
    for i, r in enumerate(type_rows):
        ...
        cells = [
            ...
            int(r[1]),
            float(r[2]),
            round(int(r[1]) / total_count * 100, 1),
        ]
    ...
    cats = db.scalars(
        select(Category).order_by(Category.order_index, Category.id)
    ).all()
    for i, cat in enumerate(cats):
        rows = db.execute(
            select(
                Nominee.id, Nominee.name, func.count(Vote.id).label("votes")
            )
        ...
        leader_name = (
            str(leader.name) if leader and int(leader.votes) > 0 else "—"
        )
    ...
    spend_rows = db.execute(
        select(
            ...
            func.coalesce(func.sum(Ticket.price), 0).label("tot"),
        ).group_by(Ticket.buyer_email)
    ).all()
    spend_map = {
        row.buyer_email: (int(row.cnt), float(row.tot)) for row in spend_rows
    }
