...
def migrate() -> None:
    ...
    try:
        ...
        if gala is None:
            print(
                "[ABORT] Aucun gala dans la base. Lance d'abord : python -m app.seed"
            )
            return

        print(
            f"[INFO] Gala cible : #{gala.id} '{gala.name}' (edition {gala.edition_year})"
        )

        # 2. Statistiques avant
        existing = db.scalars(
            select(Category).where(Category.gala_id == gala.id)
        ).all()
        ...
        for cat in existing:
            for nom in db.scalars(
                select(Nominee).where(Nominee.category_id == cat.id)
            ).all():
                # supprimer les votes lies au nominee
                for v in db.scalars(
                    select(Vote).where(Vote.nominee_id == nom.id)
                ).all():
                    db.delete(v)
                db.delete(nom)
            db.delete(cat)
        ...
        db.commit()
        print(
            f"[OK] {len(CATEGORIES_OFFICIELLES)} nouvelles categories inserees."
        )
