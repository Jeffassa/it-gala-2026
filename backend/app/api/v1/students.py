...
def list_promotions(db: Session = Depends(get_db)) -> list[str]:
    rows = db.execute(
        select(Student.promotion).distinct().order_by(Student.promotion)
    ).all()
...
def list_classes(db: Session = Depends(get_db)) -> list[str]:
    rows = db.execute(
        select(Student.classe)
        .distinct()
        .where(Student.classe.is_not(None))
        .order_by(Student.classe)
    ).all()
    return [r[0] for r in rows if r[0]]
...
def create_student(
    ...
    if db.scalar(
        ...
        raise HTTPException(
            status_code=400, detail="Matricule déjà enregistré"
        )
...
async def import_students(
    ...
    promotion: str | None = Query(
        None,
        description="Promotion par défaut si la colonne est absente du fichier",
    ),
    classe: str | None = Query(
        None,
        description="Classe par défaut si la colonne est absente du fichier",
    ),
    db: Session = Depends(get_db),
) -> StudentImportResult:
    if not file.filename.lower().endswith((".xlsx", ".xlsm")):
        raise HTTPException(
            status_code=400,
            detail="Format non supporté — uploadez un fichier .xlsx",
        )
    content = await file.read()
    if len(content) > 10 * 1024 * 1024:
        raise HTTPException(
            status_code=400, detail="Fichier trop volumineux (>10 Mo)"
        )
    try:
        result = import_students_from_xlsx(
            db, content, default_promotion=promotion, default_classe=classe
        )
    except Exception as exc:  # noqa: BLE001
        raise HTTPException(
            status_code=400, detail=f"Erreur de lecture : {exc}"
        )
